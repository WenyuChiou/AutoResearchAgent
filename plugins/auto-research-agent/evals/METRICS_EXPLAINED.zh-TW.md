...
**最新執行說明：**[evaluator v3.1](stage1/EVALUATOR_V3_1.zh-TW.md)修復證據取得、
引文選擇與恢復執行；P1–P3 十項 v3 判準沒有改。搜尋做過、文章找得到、文章內容支持結論，
是三件不同的事。原生紀錄也能作證，不因 B 多一份表格就加分；看不到的內容仍是未知。

這套指標回答一個簡單問題：研究 agent 的答案看起來很完整時，我們怎麼知道它真的可靠？

**版本先看清楚：**下文的 frozen core／must-have 命中率是歷史 v1／v2 題目專用，
不是目前通用 Stage 1 v3 的分數。新實驗採用
[無論文答案表 v3](stage1/GOLD_SET_FREE_V3.zh-TW.md)：A 是原生 Codex，B 是同一原生 Codex
加上 AutoResearchAgent；兩邊使用相同題目、模型與原生搜尋工具。v3 不要求 Eric
先列出「標準必讀論文」，而是先凍結題目需要回答的知識問題，再讓獨立評估器搜尋和核對證據。
它只能判斷**有界搜尋下的文獻充分性**，不能宣稱找齊全領域所有核心論文。

每個子指標的分子、分母與裁定規則請見 [評估指標操作型定義](OPERATIONAL_DEFINITIONS.zh-TW.md)。

## 先理解評分方式

歷史版本由兩位評分者獨立給 `0–2` 分；v3 由兩個隔離的 AI judges 獨立評分，
有實質分歧才交給第三個 AI judge 裁定。Eric 決定 rubric 與對外主張，日常 v3
評分不要求他逐篇找文獻或簽署答案表。`0` 是觀察到的缺失；來源不可取得或評估器故障
必須標為未知／失敗，不能偷算成 `0` 或 `1`。`1` 表示部分充分，`2` 表示有足夠證據。
各指標分開報告，不把 P1–P9 加成一個總分。

- **硬量測**：電腦可以直接數或檢查，例如找到幾篇、多少 claim 有原文位置、紀錄是否符合 schema。
- **研究判斷**：v3 由獨立 AI judges 按人定義的 rubric 和實際來源判斷，例如某篇文獻是否真的支持 agent 寫的結論；這是有界、可追查的判斷，不是領域真值。
- **重大錯誤門檻**：若捏造文獻、錯配 DOI、用沒有證據的「從來沒有人做過」當主要創新，或把未完成結果說成完成，該次結果不能宣稱改善。

## Stage 1：文獻蒐集與證據

目前通用 v3 的 P1 有 3 項、P2 有 4 項、P3 有 3 項，逐項 0／1／2。
完整判準、分母、未知處理和 A/B 規則見
[v3 中文操作說明](stage1/GOLD_SET_FREE_V3.zh-TW.md)。以下舊案例的篇數僅作歷史比較。
